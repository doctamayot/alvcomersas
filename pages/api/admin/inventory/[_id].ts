...
import { Part, Inventory, Movimiento } from "../../../../models";
...
const movimiento = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    ...
    if (tipo === "entrada") {
      const partUpdated = await Inventory.findByIdAndUpdate(
        _id,
        {
...
const createMov = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  try {
    ...
    const product = new Part(req.body);
    await product.save();
...
const deleteProduct = async (
  ...
  try {
    ...
    const product = await Inventory.findByIdAndDelete(_id);
    if (!product) {
      await db.disconnect();
      return res
        .status(400)
